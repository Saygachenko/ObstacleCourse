...
void AMovingPlatform::Tick(float DeltaTime)
{
	...
	const float DistanceMoved = FVector::Dist(StartLocation, CurrentLocation); // V1 = (X = -15700.0, Y = -2720.0, Z = 4690.0) V2 = (X = -15698.34, Y = -2720.0, Z = 4690.0). (-15700.0 - -15698.34)^2 + (-2720.0 - -2720.0)^2 + (4690.0 - 4690.0)^2 = дистанция между векторами (изначально мы отнимаем внутри скобок, потом умножаем саму на себя ). Вектор CurrentLocation движется бесконечно по оси Х у дистанции нет ограничения
		// Reverse direction of motion of gone too far
	if (DistanceMoved > MovedDistance) // Наше ограничения движения куба по оси X. Если наш Куб ушёл дальше от стартовой точки чем мы ограничили в MovedDistance (100) тогда...
	{
		FString NameTraps = GetName();
		float OverShot = DistanceMoved - MovedDistance; // узнаём на сколько наш куб привысил дистанцию
		UE_LOG(LogTemp, Display, TEXT("%s Platform overshot by: %f"), *NameTraps, OverShot); // выводим значение в логи
